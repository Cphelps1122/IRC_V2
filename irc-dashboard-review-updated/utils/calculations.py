...
def year_over_year_cost_trend(df: pd.DataFrame, current_month) -> pd.DataFrame:
    """Return a current-year vs prior-year monthly cost comparison.

    This is intentionally based on reporting Month + Year, not Billing Date. The
    current year is shown through the selected completed month to avoid a sharp
    drop caused by incomplete current-month bills.
    """
    if df.empty or current_month is None:
        return pd.DataFrame(columns=["Month", "Current Year", "Prior Year"])
    cm = pd.to_datetime(current_month).to_period("M").to_timestamp()
    d = df.dropna(subset=["billing_month"]).copy()
    if d.empty:
        return pd.DataFrame(columns=["Month", "Current Year", "Prior Year"])
    d["year"] = d["billing_month"].dt.year
    d["month_num"] = d["billing_month"].dt.month
    current_year = int(cm.year)
    prior_year = current_year - 1
    max_month = int(cm.month)
    d = d[d["year"].isin([current_year, prior_year])]
    # Show both years only through the selected reporting month for apples-to-apples comparison.
    d = d[d["month_num"] <= max_month]
    if d.empty:
        return pd.DataFrame(columns=["Month", str(current_year), str(prior_year)])
    pivot = (
        d.groupby(["month_num", "year"], as_index=False)["amount"].sum()
        .pivot(index="month_num", columns="year", values="amount")
        .reindex(range(1, max_month + 1))
    )
    out = pd.DataFrame({"month_num": range(1, max_month + 1)})
    out["Month"] = pd.to_datetime(dict(year=2000, month=out["month_num"], day=1)).dt.strftime("%b")
    out[str(current_year)] = pivot.get(current_year, pd.Series(index=pivot.index, dtype=float)).values if not pivot.empty else np.nan
    out[str(prior_year)] = pivot.get(prior_year, pd.Series(index=pivot.index, dtype=float)).values if not pivot.empty else np.nan
    return out[["Month", str(current_year), str(prior_year)]]


def missing_treatment_records(df: pd.DataFrame, month=None) -> pd.DataFrame:
    """Client-safe list of properties/utilities excluded from anomaly scoring due to missing treatments."""
    d = df.copy()
    if month is not None and "billing_month" in d.columns:
        d = d[d["billing_month"] == pd.to_datetime(month)]
    if d.empty:
        return pd.DataFrame(columns=["Property", "State", "Utility", "Reason"])
    uagg = utility_aggregate(d)
    if uagg.empty:
        return pd.DataFrame(columns=["Property", "State", "Utility", "Reason"])
    missing = uagg[(uagg["treatments"].isna()) | (uagg["treatments"] <= 0)].copy()
    if missing.empty:
        return pd.DataFrame(columns=["Property", "State", "Utility", "Reason"])
    missing = missing[["property", "state", "utility"]].drop_duplicates().rename(columns={"property": "Property", "state": "State", "utility": "Utility"})
    missing["Reason"] = "No treatment count entered for selected month"
    return missing.sort_values(["State", "Property", "Utility"]).reset_index(drop=True)
