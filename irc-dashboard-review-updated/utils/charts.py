...
def base_layout(fig, height=360):
    ...
    fig.update_layout(
        ...
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1, font=dict(color=t["text"])),
    )
...
def geographic_figure(state_df: pd.DataFrame, property_df: pd.DataFrame):
    ...
    if property_df is not None and not property_df.empty and {"lat", "lon"}.issubset(property_df.columns):
        color_map = {"Critical": "#EF4444", "Review": "#F59E0B", "Info": "#60A5FA", "Normal": "#22C55E", "Missing Bill": "#EF4444"}
        for status, g in property_df.dropna(subset=["lat", "lon"]).groupby("Status"):
            fig.add_trace(go.Scattergeo(
                ...
                mode="markers",
                name=status,
                marker=dict(size=9, color=color_map.get(status, "#60A5FA"), line=dict(width=1, color="white"), opacity=0.92),
    ...
    fig.update_layout(
        ...
        legend=dict(orientation="h", yanchor="bottom", y=0.01, xanchor="left", x=0.01, font=dict(color=t["text"])),
    )
    return fig
