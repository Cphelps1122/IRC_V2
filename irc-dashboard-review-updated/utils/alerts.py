...
def build_alerts(df: pd.DataFrame, current_month, previous_month=None) -> pd.DataFrame:
    ...
    for _, r in merged.iterrows():
        ...
        # Treatment volume is the operational driver. If either current or
        # previous treatments are missing/zero, there is not enough context to
        # score a true anomaly, so leave it off the alert list instead of
        # incorrectly marking it critical.
        current_treatments = r.get("treatments")
        previous_treatments = r.get("prev_treatments")
        if (pd.isna(current_treatments) or float(current_treatments or 0) <= 0 or
                pd.isna(previous_treatments) or float(previous_treatments or 0) <= 0):
            continue
    ...
    for _, combo in missing.iterrows():
        ...
        # If the latest bill on file has no treatment count, do not create a
        # missing-bill alert for V1. The item needs data completion first, not
        # anomaly scoring.
        if pd.isna(latest_treatments) or float(latest_treatments or 0) <= 0:
            continue
